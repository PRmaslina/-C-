...
int main(){
	...
	while (choise != 4) {
		...
		string menu[5] = { "1) Ввести заново                                    "
						 ...
						 ,"0) Выйти                                            " };
		bool UsualNot = checkUsualNot(expression),
			ReversePolNot = checkReversePolNot(expression),
			DirectPolNot = checkDirectPolNot(expression);
		if (!UsualNot) {
			menu[1] += "(недоступно)";
		}
		if (!ReversePolNot) {
		...
		case 2:
			system("cls");
			solutionReversePolNot(expression);
			getchar();
			break;
		case 3:
			system("cls");
			solutionDirectPolNot(expression);
			getchar();
			break;
		}
	}
